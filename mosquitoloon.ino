...
const int mainPin = 20; // Main pin cutdown
const int main_alt = 15000; // Altitude at which main cutsdown
...
void loop() {
  ...
  //bno.getEvent(&event);
  float x = 0.0;//event.orientation.x;
  float y = 0.0;//event.orientation.y;
  float z = 0.0;//event.orientation.z;

  // Getting data from thermocouple
  double int_temp = 0.0; //thermocouple.readInternal();
  double tc_temp = 0.0; //thermocouple.readCelsius();
